...
let toStoreList = JSON.parse(localStorage.getItem('storedTodo')) ? JSON.parse(localStorage.getItem('storedTodo')) : [];

//--------------------functions---------------------//
// Show LIST ITEM to DOM
const updateDOM = (toStoreList) => {
    toStoreList.forEach((todoItemObject) => {
        // CREATE LI
        ...
        todoItemObject.completed ? checkIcon.classList.add('fas', 'fa-undo-alt'): checkIcon.classList.add('fas', 'fa-check-circle');   
        // TRASH BUTTON
...
const showChoseList = (choseList) => {
    ...
    switch (choseList) {
        // UNCOMPLETED
        case 'uncompleted':
            const uncompletedList = toStoreList.filter(toStoreItem => !toStoreItem.completed);
            updateDOM(uncompletedList);
            break;
        // COMPLETED
        case 'completed':
            const completedList = toStoreList.filter(toStoreItem => toStoreItem.completed);
            updateDOM(completedList);
            break;
        //ALL
        default:
            updateDOM(toStoreList);
    }
}
